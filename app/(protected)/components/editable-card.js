...
const EditableCard = ({ title, description, canEdit, initiallyOpen = false, editLabel = 'Modifier', keepReadContent = false, children, formComponent: Form, formProps = {}, onSaved, titleIcon, creation = false, editingLabel = 'Modification en cours', className = '' }) => {
  const titleId = useId();
  ...
  useLayoutEffect(() => {
    ...
    containerRef.current?.querySelector('form input:not([type="hidden"]), form select, form textarea')?.focus();
  ...
  return (
    <section aria-labelledby={titleId} className={`${styles.card} ${editing ? styles.editing : ''} ${creation ? styles.creation : ''} ${className}`} ref={containerRef}
      onChangeCapture={() => {
        if (activeRef.current) activeRef.current.dirty = serializeFields(containerRef.current) !== baselineRef.current;
      }}>
      ...
      {editing && <Form {...formProps} onCancel={discard} onSuccess={onSuccess} onPending={onPending} />}
