...
import { addProductPackaging } from './packaging-actions.js';
...
const PackagingForm = ({ baseUnitLabel, productId }) => {
  const addPackagingWithProductId = addProductPackaging.bind(null, productId);
  const quantityUnitLabel = `${baseUnitLabel.toLocaleLowerCase('fr')}s`;
  const [state, formAction, pending] = useActionState(
    addPackagingWithProductId,
    INITIAL_STATE,
  );
  ...
  return (
    <form
      ...
      key={state.revision}
    >
      <h3 className='font-semibold text-slate-900'>Ajouter un conditionnement</h3>
      <p className='mt-1 text-sm leading-6 text-slate-600'>
        Indiquez combien d’unités de stock contient ce conditionnement.
      </p>
      ...
      <div className='mt-4 grid gap-4 sm:grid-cols-2'>
        ...
        <div>
          ...
          {state.errors.quantity && (
            <p className='mt-2 text-sm text-red-700' id='packaging-quantity-error'>
              {state.errors.quantity}
            </p>
          )}
        </div>
      ...
      <button
        className='mt-5 rounded-lg bg-blue-700 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-blue-800 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-blue-700 disabled:cursor-not-allowed disabled:opacity-60'
        disabled={pending}
        type='submit'
      >
