...
export const getReceptionTabs = ({ canReadReceptions, canReadSuppliers }) => [
  ...(canReadReceptions ? [{ key: 'receptions', label: 'Réceptions' }] : []),
  ...(canReadSuppliers ? [{ key: 'fournisseurs', label: 'Fournisseurs' }] : []),
];
...
export const validateReceptionHistoryHref = (value) => {
  ...
  if (
    ...
    || (destination.searchParams.has(TAB_PARAMETER)
      && destination.searchParams.get(TAB_PARAMETER) !== 'receptions')
  ) {
    return '/receptions';
  }
