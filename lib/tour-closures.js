...
const createTourClosureDigest = ({ counting, payments }) => createHash('sha256')
  .update(JSON.stringify({
    ...
    payments: payments
      .map((payment) => ({
        ...
        sourceTourCountingId: payment.sourceTourCountingId?.toString?.() ?? '',
      }))
...
const readTourClosureSummary = async ({ database, session, tour }) => {
  ...
  const [counting, payments] = await Promise.all([
    database.collection('tourCountings').findOne(
      { _id: tour.countingId, tourId: tour._id },
      { projection: { totalDueInCentimes: 1 }, session },
    ),
    readStoredPayments({ database, session, tourId: tour._id }),
  ]);
  ...
  return {
    ...
    digest: createTourClosureDigest({ counting, payments }),
