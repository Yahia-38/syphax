...
// The default sale unit follows the usages: the migration may reset it, so it is
// excluded from the staleness fingerprint like the usages themselves.
const withoutUsages = ({ defaultSaleUnit, ...product }) => ({
  ...product,
...
export const applyPackagingUsageInventory = async (database, client, inventory, saveBackup) =>
  client.withSession(async (session) => session.withTransaction(async () => {
    ...
    for (const update of updates) {
      ...
      if (!isPackagingEnabledForSale(update)) {
        // A packaging that is no longer sold cannot remain the default sale unit.
        await database.collection('products').updateOne(
          { _id: update.productId, defaultSaleUnit: update.packagingId },
          { $set: { defaultSaleUnit: null } },
          { session },
        );
      }
    }
    ...
    const expected = products.map((product) => ({
      ...product,
      ...(updates.some((update) => update.productId.equals(product._id)
        && !isPackagingEnabledForSale(update)
        && product.defaultSaleUnit?.equals?.(update.packagingId))
