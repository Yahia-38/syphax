...
export const TOUR_RETURN_INPUT_KIND = 'TOUR_RETURN_IN';
...
export const ensureStockMovementIndexes = async (database) => {
  ...
  await Promise.all([
    ...
    stockMovements.createIndex(
      ...
      {
        name: 'unique_tour_return_reservation_movement',
        partialFilterExpression: { kind: TOUR_RETURN_INPUT_KIND },
        unique: true,
      },
    ),
    stockMovements.createIndex(
...
export const listProductStockMovements = async (
  productId,
  { database: providedDatabase, session, includeTourSources = false, includeReceptionSources = false } = {},
) => {
  ...
  const documents = await database.collection('stockMovements').find(
    { productId },
    {
      ...
      projection: {
        ...
        recordedAt: 1,
        recordedBy: 1,
        ...(includeReceptionSources ? { sourceReceptionId: 1 } : {}),
  ...
  const receptionIds = includeReceptionSources ? documents
    .map((movement) => movement.sourceReceptionId)
    .filter((id) => id instanceof ObjectId) : [];
  const receptions = receptionIds.length ? await database.collection('receptions').find(
    { _id: { $in: receptionIds } }, { projection: { supplierReference: 1 }, session },
  ).toArray() : [];
  const receptionsById = new Map(receptions.map((reception) => [reception._id.toString(), reception]));
  ...
  return documents.map((movement) => ({
    ...
    sourceReception: includeReceptionSources
      && receptionsById.has(movement.sourceReceptionId?.toString())
      ? { id: movement.sourceReceptionId.toString(), reference: receptionsById.get(movement.sourceReceptionId.toString()).supplierReference ?? 'Réception' }
      : null,
