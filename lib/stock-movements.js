...
export const getProductStockSummaries = async (
  ...
  const database = providedDatabase ?? await getDatabase();
  const summaries = await database.collection('stockMovements').aggregate([
    {
      $match: {
  ...
  return new Map(summaries.map((summary) => [
    ...
    {
      inputQuantityInBaseUnits: summary.inputQuantityInBaseUnits,
      lastMovementAt: summary.lastMovementAt ?? null,
      movementCount: summary.movementCount,
      outputQuantityInBaseUnits: summary.outputQuantityInBaseUnits,
      quantityInBaseUnits: summary.quantityInBaseUnits,
    },
  ]));
};
