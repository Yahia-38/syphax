...
const serializeCashRemainder = (remainder) => ({
  deliverer: {
...
export const listCashRemainders = async ({
  ...
  const database = await getDatabase();
  let result = await runCashRemaindersAggregation({
    database,
    limit: normalizedPageSize,
    page: state.page,
    query: state.query,
  ...
  return {
    ...
    pageSize: normalizedPageSize,
    query: state.query,
    remainders: (result.items ?? []).map(serializeCashRemainder),
    totalItems,
    totalPages,
