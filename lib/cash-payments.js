...
import { readCashTab } from './cash-navigation.js';
...
export const createAlgiersDayBoundary = (value, nextDay = false) => {
...
export const buildCashJournalHref = ({
  ...
  if (view !== undefined) parameters.set(TAB_PARAMETER, readCashTab(view));
...
export const validateCashJournalHref = (value) => {
  ...
  return buildCashJournalHref({
    ...
    view: searchParams[TAB_PARAMETER] === undefined
      ? undefined : readCashTab(searchParams[TAB_PARAMETER]),
  });
};
...
const serializeCashJournalPayment = (payment) => {
  const normalizedPayment = normalizeStoredPaymentAllocations(payment);
  const isWithdrawal = payment.journalType === 'WITHDRAWAL';
  const allocations = !isWithdrawal && normalizedPayment
  ...
  return {
    allocations,
    amountInCentimes: Number.isSafeInteger(payment.amountInCentimes)
      && payment.amountInCentimes > 0
      && payment.currency === CASH_CURRENCY
    ...
    receivedAt: payment.occurredAt?.toISOString?.() ?? null,
...
const createCashJournalFilter = ({
  ...
  ...(dateFrom || dateTo
    ? {
        [withdrawal ? 'withdrawnAt' : 'receivedAt']: {
          ...(dateFrom
...
export const listCashPayments = async ({
  ...
  const paymentFilter = createCashJournalFilter(state);
  const withdrawalFilter = createCashJournalFilter(state, {
    withdrawal: true,
  });
  const database = await getDatabase();
  const cashPayments = database.collection('cashPayments');
  const createCombinedJournalPipeline = () => [
    { $match: paymentFilter },
    {
      $set: {
        authorId: '$receivedBy',
    ...
    {
      $unionWith: {
        coll: 'cashWithdrawals',
        pipeline: [
          { $match: withdrawalFilter },
          {
            $set: {
              authorId: '$withdrawnBy',
              ...
              invalidFinancialItems: {
                $sum: {
                  $cond: [
                    {
                      $and: [
                        { $isNumber: '$amountInCentimes' },
                        { $gt: ['$amountInCentimes', 0] },
                        { $eq: ['$currency', CASH_CURRENCY] },
                      ],
                    },
                    0,
                    1,
  ...
  const summary = result.summary?.[0] ?? {
    cashRegisters: [],
    invalidFinancialItems: 0,
    totalEntriesInCentimes: 0,
    totalItems: 0,
    totalWithdrawalsInCentimes: 0,
  };
  ...
  const totalsAreValid = summary.invalidFinancialItems === 0
    && Number.isSafeInteger(summary.totalEntriesInCentimes)
    && summary.totalEntriesInCentimes >= 0
    && Number.isSafeInteger(summary.totalWithdrawalsInCentimes)
    && summary.totalWithdrawalsInCentimes >= 0;
  const totalEntriesInCentimes = totalsAreValid
    ? summary.totalEntriesInCentimes
    : null;
  const totalWithdrawalsInCentimes = totalsAreValid
    ? summary.totalWithdrawalsInCentimes
    : null;
  ...
  return {
    ...
    financialDataError: totalsAreValid
      ? null
      : 'La sélection contient une donnée financière incohérente.',
    netVariationInCentimes: totalsAreValid
      ? totalEntriesInCentimes - totalWithdrawalsInCentimes
      : null,
    totalAmountInCentimes: totalEntriesInCentimes,
    totalEntriesInCentimes,
    totalItems,
    totalPages,
    totalWithdrawalsInCentimes,
...
export const loadCashRemainderRecords = async (
  database,
  session,
  { delivererId } = {},
) => {
  ...
  const expenseDeclarations = await database.collection('tourExpenses').find(
    { tourId: { $in: tourIds } },
    { session },
  ).toArray();
  ...
  const expenseDeclarationsByTourId = new Map(expenseDeclarations.map(
    (declaration) => [declaration.tourId?.toString?.(), declaration],
  ));
  const records = tours.map((tour) => {
    ...
    const storedDeclaration = expenseDeclarationsByTourId.get(
      tour._id.toString(),
    );

    if (!anomaly && storedDeclaration) {
      const declaration = normalizeStoredTourExpenseDeclaration(
        storedDeclaration,
    ...
    const netDueInCentimes = anomaly
      ? null
      : counting.totalDueInCentimes - totalExpensesInCentimes;

    if (
      !anomaly
      && (!Number.isSafeInteger(netDueInCentimes) || netDueInCentimes < 0)
    ) {
      anomaly = 'INVALID_EXPENSE_DECLARATION';
    }
    ...
    return {
      ...tour,
      amountDueInCentimes: anomaly ? null : netDueInCentimes,
      amountPaidInCentimes: 0,
      anomaly,
      countedAt: counting?.countedAt ?? null,
      expenseDeclarationStatus,
      grossSalesInCentimes: anomaly ? null : counting.totalDueInCentimes,
      invalidPayment: false,
      netDueInCentimes: anomaly ? null : netDueInCentimes,
      totalExpensesInCentimes: anomaly ? null : totalExpensesInCentimes,
    };
  });
...
const serializeCashRemainder = (remainder, blockingAnomalies = []) => ({
  ...
  tours: remainder.tours.map((tour) => ({
    ...
    grossSalesInCentimes: readAggregatedSafeInteger(
      tour.grossSalesInCentimes,
    ),
    id: tour.id.toString(),
    netDueInCentimes: readAggregatedSafeInteger(tour.netDueInCentimes),
    reference: tour.reference ?? 'Référence indisponible',
    remainingDueInCentimes: readAggregatedSafeInteger(
      tour.remainingDueInCentimes,
    ),
    status: tour.status,
    totalExpensesInCentimes: readAggregatedSafeInteger(
      tour.totalExpensesInCentimes,
    ),
  })),
});
...
export const summarizeCashRemainderRecords = (records) => {
  if (records.length === 0) {
    return {
  ...
  const expenseDeclarationsMissingCount = records.filter((record) =>
    record.expenseDeclarationStatus
      === TOUR_EXPENSE_DECLARATION_STATUS_MISSING).length;
  const historicalExpenseDeclarationsMissingCount = records.filter((record) =>
    record.expenseDeclarationStatus
      === TOUR_EXPENSE_DECLARATION_STATUS_HISTORICAL_MISSING).length;
  ...
  if (anomalies.length > 0) {
    return {
      ...
      expenseDeclarationsMissingCount,
      grossSalesInCentimes: null,
      historicalExpenseDeclarationsMissingCount,
      netDueInCentimes: null,
      reliable: false,
      remainingDueInCentimes: null,
      totalExpensesInCentimes: null,
  ...
  const totals = records.reduce((summary, record) => ({
    ...
    grossSalesInCentimes:
      summary.grossSalesInCentimes + record.grossSalesInCentimes,
    netDueInCentimes:
      summary.netDueInCentimes + record.netDueInCentimes,
    remainingDueInCentimes:
      summary.remainingDueInCentimes + record.remainingDueInCentimes,
    totalExpensesInCentimes:
      summary.totalExpensesInCentimes + record.totalExpensesInCentimes,
  }), {
...
export const readDelivererCashSummary = async ({
  database,
  delivererId,
  session,
}) => summarizeCashRemainderRecords(
  await loadCashRemainderRecords(database, session, { delivererId }),
);
...
export const getDelivererCashSummary = async ({ delivererId, userId } = {}) => {
  ...
  try {
    summary = await session.withTransaction(
...
export const listCashRemainders = async ({
  ...
  for (const record of filteredRecords) {
    ...
    remainder.tours.push({
      ...
      expenseDeclarationStatus: record.expenseDeclarationStatus,
      grossSalesInCentimes: record.grossSalesInCentimes,
      id: record._id,
      netDueInCentimes: record.netDueInCentimes,
      reference: tourReference,
      remainingDueInCentimes: record.remainingDueInCentimes,
      status: record.status,
      totalExpensesInCentimes: record.totalExpensesInCentimes,
    });
...
export const getTourPaymentPreview = async ({ tourId, userId }) => {
  ...
  const [cashRegisters, resolvedPayments, expenseResolution] = await Promise.all([
  ...
  const grossSalesInCentimes = tourAndCounting.counting.totalDueInCentimes;
  const totalExpensesInCentimes =
    expenseResolution?.totalExpensesInCentimes ?? null;
  const amountDueInCentimes = totalExpensesInCentimes === null
    ? null
  ...
  if (
    !expenseResolution
    || amountPaidInCentimes === null
    || !Number.isSafeInteger(amountDueInCentimes)
    || amountDueInCentimes < 0
  ...
  const expenseDeclarationStatus = expenseResolution.declaration
    ? TOUR_EXPENSE_DECLARATION_STATUS_DECLARED
  ...
  return {
    ...
    grossSalesInCentimes,
    netDueInCentimes: amountDueInCentimes,
...
export const recordTourCashPayment = async ({
  ...
  try {
    return await session.withTransaction(async () => {
      ...
      const [previousPayments, expenseResolution] = await Promise.all([
        readStoredTourPaymentResolution({
          database,
      ...
      const netDueInCentimes = expenseResolution
