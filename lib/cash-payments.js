...
const createCashRemaindersPipeline = ({ limit, query, skip }) => {
  const delivererSearch = query
    ? {
        $match: {
          $or: [
            {
    ...
    : null;
  const validStoredPaymentExpression = {
    $and: [
      {
        $in: [
          { $type: '$amountInCentimes' },
          ['int', 'long', 'double'],
        ],
      },
      { $ne: ['$amountForValidation', null] },
      { $eq: ['$amountForValidation', { $trunc: '$amountForValidation' }] },
      { $gt: ['$amountForValidation', 0] },
      { $lte: ['$amountForValidation', Number.MAX_SAFE_INTEGER] },
      { $eq: ['$currency', CASH_CURRENCY] },
      { $eq: ['$sourceTourCountingId', '$$countingId'] },
    ],
  };
  const groupedRemaindersPipeline = [
    {
      $match: {
        anomaly: null,
        remainingDueInCentimes: { $gt: 0 },
      },
    },
    { $sort: { _id: -1 } },
    {
      $group: {
        _id: '$delivererId',
        delivererCode: { $first: '$delivererCode' },
        delivererName: { $first: '$delivererName' },
        remainingDueInCentimes: { $sum: '$remainingDueInCentimes' },
        tourCount: { $sum: 1 },
        tours: {
          $push: {
            amountDueInCentimes: '$amountDueInCentimes',
            amountPaidInCentimes: '$paymentSummary.amountPaidInCentimes',
    ...
    ...(delivererSearch ? [delivererSearch] : []),
  ];
  ...
  return [
    ...
    },
    {
      $lookup: {
        as: 'countings',
        from: 'tourCountings',
        let: { countingId: '$countingId', tourId: '$_id' },
        pipeline: [
          {
            $match: {
              $expr: {
          ...
          { $project: { countedAt: 1, totalDueInCentimes: 1 } },
        ],
      },
    },
    {
      $lookup: {
        ...
        let: { countingId: '$countingId', tourId: '$_id' },
        pipeline: [
          { $match: { $expr: { $eq: ['$tourId', '$$tourId'] } } },
          {
            $set: {
              amountForValidation: {
                $convert: {
                  input: '$amountInCentimes',
                  onError: null,
                  onNull: null,
                  to: 'decimal',
                },
          ...
          {
            $set: {
              validStoredPayment: validStoredPaymentExpression,
            },
          },
          {
            $group: {
              ...
              amountPaidInCentimes: {
                $sum: {
                  $cond: [
                    '$validStoredPayment',
                    { $convert: { input: '$amountForValidation', to: 'long' } },
                    0,
                  ],
                },
              },
              invalidPaymentCount: {
                $sum: { $cond: ['$validStoredPayment', 0, 1] },
              },
            },
          },
        ],
    ...
    {
      $set: {
        counting: { $arrayElemAt: ['$countings', 0] },
        paymentSummary: {
          $ifNull: [
            { $arrayElemAt: ['$paymentSummaries', 0] },
            { amountPaidInCentimes: 0, invalidPaymentCount: 0 },
          ],
        },
      },
    },
    {
      $set: {
        amountDueForValidation: {
          $convert: {
            input: '$counting.totalDueInCentimes',
            onError: null,
            onNull: null,
            to: 'decimal',
          },
      ...
      $set: {
        validCountingAmount: {
          $and: [
            {
              $in: [
                { $type: '$counting.totalDueInCentimes' },
                ['int', 'long', 'double'],
              ],
            },
            { $ne: ['$amountDueForValidation', null] },
            {
              $eq: [
                '$amountDueForValidation',
                { $trunc: '$amountDueForValidation' },
              ],
            },
            { $gte: ['$amountDueForValidation', 0] },
            { $lte: ['$amountDueForValidation', Number.MAX_SAFE_INTEGER] },
          ],
        },
        validPaymentTotal: {
          $and: [
            { $eq: ['$paymentSummary.invalidPaymentCount', 0] },
            {
              $lte: [
                '$paymentSummary.amountPaidInCentimes',
    ...
    },
    {
      $set: {
        amountDueInCentimes: {
          $cond: [
            '$validCountingAmount',
            { $convert: { input: '$amountDueForValidation', to: 'long' } },
            null,
          ],
        },
      },
      ...
      $set: {
        anomaly: {
          $switch: {
            branches: [
              {
                case: { $ne: [{ $type: '$delivererId' }, 'objectId'] },
                then: 'INVALID_DELIVERER',
              },
              {
                case: {
                  $or: [
                    { $ne: [{ $type: '$countingId' }, 'objectId'] },
                    { $eq: [{ $size: '$countings' }, 0] },
                  ],
                },
                then: 'MISSING_COUNTING',
              },
              {
                case: {
                  $or: [
                    { $ne: [{ $size: '$countings' }, 1] },
                    { $eq: ['$validCountingAmount', false] },
                  ],
                },
                then: 'INVALID_COUNTING',
              },
              {
                case: { $eq: ['$validPaymentTotal', false] },
                then: 'INVALID_PAYMENTS',
              },
              {
                case: {
                  $gt: [
          ...
          $cond: [
            {
              $and: [
                { $eq: ['$anomaly', null] },
                { $ne: [{ $type: '$counting.countedAt' }, 'date'] },
              ],
            },
            'MISSING_COUNTING_DATE',
            '$anomaly',
          ],
        },
      },
    },
    {
      $facet: {
        allocationAnomalies: [
          { $match: { allocationAnomaly: { $ne: null } } },
          ...(delivererSearch ? [delivererSearch] : []),
          { $match: { delivererId: { $type: 'objectId' } } },
          {
            $group: {
              ...
              anomalies: {
                $push: {
                  code: '$allocationAnomaly',
                  tourReference: {
                    $ifNull: ['$reference', 'Référence indisponible'],
                  },
                },
              },
            },
        ...
        anomalies: [
          ...
          ...(delivererSearch ? [delivererSearch] : []),
          {
            $group: {
              ...
              count: { $sum: 1 },
              tourReferences: {
                $addToSet: { $ifNull: ['$reference', 'Référence indisponible'] },
              },
            },
          },
          { $sort: { _id: 1 } },
        ...
        items: [
          ...groupedRemaindersPipeline,
          { $sort: { delivererCode: 1, delivererName: 1, _id: 1 } },
          { $skip: skip },
          { $limit: limit },
        ],
        summary: [
...
const runCashRemaindersAggregation = async ({
  database,
  limit,
  page,
  query,
}) => {
  const [result = {}] = await database.collection('tours').aggregate(
    createCashRemaindersPipeline({
      limit,
      query,
      skip: (page - 1) * limit,
    }),
...
export const listCashRemainders = async ({
  ...
  const database = await getDatabase();
  const [initialResult, activeCashRegisters] = await Promise.all([
    runCashRemaindersAggregation({
      database,
      limit: normalizedPageSize,
      page: state.page,
      query: state.query,
  ...
  const totalItems = readAggregatedSafeInteger(summary.totalItems) ?? 0;
  const totalPages = Math.max(1, Math.ceil(totalItems / normalizedPageSize));
  const activePage = Math.min(state.page, totalPages);

  if (activePage !== state.page && totalItems > 0) {
    result = await runCashRemaindersAggregation({
      database,
      limit: normalizedPageSize,
      page: activePage,
      query: state.query,
  ...
  const anomalies = (result.anomalies ?? []).map((anomaly) => ({
    code: anomaly._id,
    count: readAggregatedSafeInteger(anomaly.count) ?? 0,
    label: CASH_REMAINDER_ANOMALY_LABELS[anomaly._id]
      ?? 'données financières incohérentes',
    tourReferences: [...(anomaly.tourReferences ?? [])]
  ...
  const allocationAnomaliesByDeliverer = new Map(
    (result.allocationAnomalies ?? []).map((entry) => [
      entry._id.toString(),
      (entry.anomalies ?? []).map((anomaly) => ({
        code: anomaly.code,
        label: CASH_REMAINDER_ANOMALY_LABELS[anomaly.code]
          ?? 'données financières incohérentes',
        tourReference: typeof anomaly.tourReference === 'string'
          ? anomaly.tourReference
          : 'Référence indisponible',
      })),
    ]),
  );
  ...
  return {
    ...
    pageSize: normalizedPageSize,
    query: state.query,
    remainders: (result.items ?? []).map((remainder) =>
