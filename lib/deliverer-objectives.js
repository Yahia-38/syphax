...
export const updateDelivererObjective = async ({ amount, effectiveMonth, delivererId, expectedVersion, updatedBy }) => {
  ...
  if (!Number.isSafeInteger(expectedVersion) || expectedVersion < 0 || expectedVersion >= Number.MAX_SAFE_INTEGER) return staleResult;
  const database = await getDatabase();
  ...
  const versionFilter = expectedVersion === 0
    ? { $or: [{ objectiveVersion: { $exists: false } }, { objectiveVersion: 0 }] }
    : { objectiveVersion: expectedVersion };
