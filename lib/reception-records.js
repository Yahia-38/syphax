...
import { applyReceptionStockValuations } from './reception-stock-valuations.js';
import {
...
export const createReception = async ({
  ...
  try {
    return await session.withTransaction(async () => {
      ...
      try {
        await applyReceptionStockValuations({ database, session, lines: storedLines, movements });
      } catch (error) {
        if (error instanceof RangeError) {
          throw new ReceptionTransactionValidationError({
            lines: 'La valorisation du stock empêche cette réception. Vérifiez les unités, les quantités et les montants du stock existant.',
          });
        }
