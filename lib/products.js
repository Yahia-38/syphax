...
export const deleteProduct = async (id) => {
  ...
  const [receptionUsingProduct, stockMovement] = await Promise.all([
...
export const getProductById = async (id, { includePricing = false } = {}) => {
  if (typeof id !== 'string' || !ObjectId.isValid(id)) {
    return null;
  }
  ...
  const [users, stockSummaries] = await Promise.all([
    userIds.length > 0
