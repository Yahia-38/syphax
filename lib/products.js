...
import { PACKAGING_USAGES } from './product-packaging.js';
import { sortProductPackagings, validateProductSalePrice } from './product-pricing.js';
import {
...
export const updateProductPackagingSalePrice = async ({ productId, packagingId, price, updatedBy }) => {
  ...
  return result.matchedCount === 0
    ? { notFound: true }
    : { salePrice: { amountInCentimes, currency: 'DZD', taxIncluded: true } };
};

export const listProducts = async ({
  includePackagings = false,
  includePricing = false,
  priceByLargestPack = false,
  onlyUsable = false,
  query = '',
} = {}) => {
  ...
  const projection = {
    ...
    code: 1,
    designation: 1,
    ...(includePackagings || (includePricing && priceByLargestPack) ? { packagings: 1 } : {}),
  ...
  return documents.map((product) => {
    ...
    const largestPackaging = includePricing && priceByLargestPack
      ? sortProductPackagings(product.packagings)[0] : null;
    const salePrice = largestPackaging ? largestPackaging.salePrice : product.salePrice;
      ...
      ...(includePricing
        ? {
            salePriceCentimes: Number.isSafeInteger(
              salePrice?.amountInCentimes,
            )
              ? salePrice.amountInCentimes
              : null,
            ...(priceByLargestPack ? { salePricePackagingQuantity: largestPackaging?.quantity ?? null } : {}),
          }
        : {}),
    };
  });
