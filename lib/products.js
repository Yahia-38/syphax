...
// Default sale units keyed by product id, for screens that only hold product references.
export const getProductDisplayUnits = async (productIds) => {
  ...
  const database = await getDatabase();
  const documents = await database.collection('products')
    .find({ _id: { $in: ids } }, { projection: { defaultSaleUnit: 1, packagings: 1 } })
    .toArray();

  return new Map(documents.map((product) => [
    product._id.toString(),
    getProductDisplayUnit({
      defaultSaleUnit: readDefaultSaleUnit(product),
      packagings: (Array.isArray(product.packagings) ? product.packagings : []).map((packaging) => ({
        id: packaging._id.toString(),
        label: packaging.label,
        quantity: packaging.quantity,
        usage: packaging.usage ?? null,
