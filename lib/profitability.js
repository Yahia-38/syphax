...
export const getProfitabilityReport = async ({
  ...
  try {
    return await session.withTransaction(async () => {
      ...
      // The deliverer choices ignore the other filters so that the list stays
      // the same while the selection changes. Each deliverer is named as on
      // their most recently counted tour.
      const deliverers = await database.collection('tours').aggregate([
        {
          $match: {
            delivererId: { $type: 'objectId' },
            status: { $in: PROFITABILITY_TOUR_STATUSES },
          },
        },
      ...
      return {
        delivererOptions: deliverers.map((deliverer) => ({
          code: deliverer.code ?? '',
          id: deliverer._id.toString(),
          name: deliverer.name ?? 'Livreur inconnu',
        })),
