...
export const getTourById = async (id, { userId } = {}) => {
  ...
  const [author, lines] = await Promise.all([
    tour.createdBy instanceof ObjectId
      ...
      : null,
    listTourReservations({ database, tourId: id }),
  ]);
...
export const formatTourStatus = (status) => status === TOUR_STATUS_PREPARATION
  ? 'En préparation'
  : 'Statut inconnu';
