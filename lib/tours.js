...
import { DELIVERER_TAB_KEYS } from './deliverer-detail-navigation.js';
import { isObjectiveMonth, OBJECTIVE_ACHIEVEMENT_STATUSES, readObjectiveAchievementState, readObjectiveHistoryState } from './deliverer-objective-calculations.js';
import { getDatabase, getMongoClient } from './mongodb.js';
import { isProfitabilityHref, validateProfitabilityHref } from './profitability-navigation.js';
...
export const TOUR_STATUS_CLOSED = 'CLOSED';
export const TOUR_STATUS_CANCELLED = 'CANCELLED';
...
const ensureTourIndexes = async (database) => {
  await Promise.all([
    ...
    database.collection('tours').createIndex(
      { plannedDate: 1, delivererName: 1, createdAt: 1, _id: 1 },
      { name: 'tour_planned_day' },
    ),
  ]);
};
...
export const getTourById = async (
  ...
  const [authors, lines] = await Promise.all([
    ...
    listTourReservations({
      database,
      includePricing,
      includeReleased: tour.status === TOUR_STATUS_CANCELLED,
      tourId: id,
    }),
  ]);
  ...
  return {
    ...
    closedBy: authorsById.get(tour.closedBy?.toString()) ?? null,
    cancelledAt: tour.cancelledAt?.toISOString?.() ?? null,
    cancelledBy: authorsById.get(tour.cancelledBy?.toString()) ?? null,
    cancellationReason: tour.cancellationReason ?? null,
    lines,
  };
};
...
export const buildDelivererToursHref = ({
  ...
  if (DELIVERER_TAB_KEYS.includes(tab)) {
    parameters.set(TAB_PARAMETER, tab);
  }

  if (typeof objectiveQuery === 'string' && objectiveQuery.trim()) {
    parameters.set('objectifRecherche', objectiveQuery.trim().slice(0, 100));
  }
  if (isObjectiveMonth(objectiveMonth)) parameters.set('objectifMois', objectiveMonth);
  if (Number.isSafeInteger(objectivePage) && objectivePage > 1) {
    parameters.set('objectifPage', String(objectivePage));
  }

  if (isObjectiveMonth(achievementMonth)) parameters.set('bilanMois', achievementMonth);
  if (typeof historyYear === 'string' && /^[1-9]\d{3}$/u.test(historyYear)) parameters.set('bilanAnnee', historyYear);
  if (typeof historyStatus === 'string' && Object.hasOwn(OBJECTIVE_ACHIEVEMENT_STATUSES, historyStatus)) parameters.set('bilanStatut', historyStatus);
  if (typeof historyQuery === 'string' && historyQuery.trim()) parameters.set('bilanRecherche', historyQuery.trim().slice(0, 100));
  if (Number.isSafeInteger(historyPage) && historyPage > 1) parameters.set('bilanPage', String(historyPage));
...
export const validateTourReturnHref = (value, delivererId) => {
  ...
  // A tour is also opened from the day recap, which is a destination of its own.
  if (isDayRecapHref(value)) {
    return validateDayRecapHref(value);
  }

  // And from the profitability page, to read the counting behind its figures.
  if (isProfitabilityHref(value)) {
    return validateProfitabilityHref(value);
  }
  ...
  if (
    ...
    || (destination.searchParams.has(TAB_PARAMETER)
      && !DELIVERER_TAB_KEYS.includes(destination.searchParams.get(TAB_PARAMETER)))
  ...
  return buildDelivererToursHref({
    ...
    ...readObjectiveHistoryState(Object.fromEntries(destination.searchParams)),
    ...(destination.searchParams.has('bilanMois') || destination.searchParams.has('bilanAnnee')
      || destination.searchParams.has('bilanStatut') || destination.searchParams.has('bilanRecherche')
      || destination.searchParams.has('bilanPage')
      ? readObjectiveAchievementState(Object.fromEntries(destination.searchParams)) : {}),
    returnHref: destination.searchParams.get('retour') ?? '/livreurs',
    tab: destination.searchParams.get(TAB_PARAMETER),
  });
};

// The back link names where it leads: the origins are not interchangeable.
export const describeTourReturn = (returnHref) => {
  if (isProfitabilityHref(returnHref)) {
    return { label: '← Retour à la rentabilité', target: 'profitability' };
  }

  if (!isDayRecapHref(returnHref)) {
    return { label: '← Retour à la fiche livreur', target: 'deliverer' };
  }

  const date = formatDayRecapShortDate(readDayRecapHrefDate(returnHref));

  return {
    label: date ? `← Retour au récap du ${date}` : '← Retour au récap',
    target: 'dayRecap',
  };
};
...
// Tours still needing work: prepared, loaded, or counted but not yet closed.
export const OPEN_TOUR_STATUSES = Object.freeze([
  TOUR_STATUS_PREPARATION,
  TOUR_STATUS_LOADED,
  TOUR_STATUS_COUNTED,
]);

// The fiche's status bar shows the latest open tour and how many others remain.
export const getDelivererOpenTours = async ({ delivererId, userId } = {}) => {
  await requireUserPermission(userId, 'tours.read');
  ...
  const filter = {
    delivererId: new ObjectId(delivererId),
    status: { $in: OPEN_TOUR_STATUSES },
  };
  const tours = (await getDatabase()).collection('tours');
  const [count, latest] = await Promise.all([
    tours.countDocuments(filter),
    tours.findOne(filter, {
      projection: { plannedDate: 1, reference: 1, status: 1 },
      sort: { plannedDate: -1, createdAt: -1, _id: -1 },
    }),
  ]);
  ...
  return {
    ...
    latest: latest
      ? {
          ...
          plannedDate: formatStoredDate(latest.plannedDate),
