...
export const buildDayAlerts = (
  ...
  for (const record of records) {
    ...
    const push = (code, label) => alerts.push({
      anomaly: code === 'ANOMALY' ? record.anomaly : null,
      code,
      deliverer,
...
export const DAY_ALERT_TYPES = Object.freeze({
  ANOMALY: 'Données à vérifier',
  NOT_RETURNED: 'Retour à vérifier',
  REMAINING_DUE: 'Reste à encaisser',
  MISSING_EXPENSES: 'Frais non déclarés',
  TO_CLOSE: 'À terminer',
  PREPARATION: 'Chargement à vérifier',
});

// An inconsistency is only named to someone allowed to read its domain.
const DAY_ANOMALY_DOMAINS = Object.freeze({
  INVALID_EXPENSE_DECLARATION: 'expenses',
  INVALID_PAYMENTS: 'cash',
  OVERPAID: 'financials',
});

// The alerts are built from every record of the day: before they are shown,
// categories the reader cannot see are dropped and hidden causes are
// reduced to an operational label. Only the fields the panel needs remain.
export const projectDayAlerts = (
  alerts = [],
  { canReadCash = false, canReadExpenses = false } = {},
) => {
  const readable = {
    cash: canReadCash,
    expenses: canReadExpenses,
    financials: canReadCash && canReadExpenses,
  };

  return alerts
    .filter(({ code }) => {
      if (code === 'REMAINING_DUE') {
        return readable.financials;
      }

      // Telling a tour to close apart from missing expenses reads the
      // declaration: both stay with the expense permission.
      if (code === 'MISSING_EXPENSES' || code === 'TO_CLOSE') {
        return readable.expenses;
      }

      return Object.hasOwn(DAY_ALERT_TYPES, code);
    })
    .map((alert, index) => {
      const domain = DAY_ANOMALY_DOMAINS[alert.anomaly];
      ...
      return {
        ...
        href: alert.href,
        id: `${alert.code}-${index}`,
        label: alert.code === 'ANOMALY' && domain && !readable[domain]
          ? 'données à vérifier'
          : alert.label,
        reference: alert.tourReference,
        shortReference: formatDayTourReference(alert.tourReference),
        type: DAY_ALERT_TYPES[alert.code],
      };
    });
};
