...
export const getUserPermissions = async (
  ...
  const roles = await database.collection('roles').find(
    { _id: { $in: user.roleIds } },
    { projection: { permissions: 1 }, session },
  ).toArray();
...
export const userHasPermission = async (userId, permission, options) => {
  assertKnownPermission(permission);
  return (await getUserPermissions(userId, options)).includes(permission);
};

export const requireUserPermission = async (userId, permission, options) => {
  if (!(await userHasPermission(userId, permission, options))) {
