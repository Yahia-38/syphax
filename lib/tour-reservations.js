...
const ensureTourReservationIndexes = async (database) => {
...
const readExistingAddition = async ({
  ...
  const reservation = await database.collection('tourReservations').findOne(
    { additionKey },
    { projection: { requestDigest: 1, tourId: 1 }, session },
  );
  ...
  try {
    return await session.withTransaction(async () => {
      ...
      const existingProduct = await database
        ...
        .findOne(
          { tourId: tourObjectId, productId: productObjectId },
    ...
    if (error?.code === 11000) {
      ...
      if (
        ...
        || error?.message?.includes('unique_tour_product')
      ) {
        return {
          errors: {
            productId: 'Ce produit est déjà présent dans cette tournée.',
...
export const listTourReservations = async ({ database, tourId }) => {
